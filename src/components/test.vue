<template>
    <div id="one">
        ...
        <div class="box">
            ...
            <div class="days-cont ">
                <div :title="item.date" v-for=" item in dateObj.days" class="day-item" :class="{'today': item.date == realCurrentDate ,'current-day': item.day == currentDay && item.month == currentMonth+1, 'disable-item' : item.month == currentMonth}" @click="setDay(item)">
                    <span>{{ item.day }}</span>
                    <!--span>{{ item.date }}</span-->
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            init() { //日期初始化
                this.getNowDate()
                if (this.originSetDateValue === '' || !this.originSetDateValue) { //未设置初始日期时，默认系统当前日期
                    this.getDate()
                } else {
