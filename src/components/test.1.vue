<template>
    <div id="one">
        <div id="name" ref="picker">点我选择</div>
        <input type="text" v-model="setDateValue" @change="setDate(setDateValue)">
        <div class="box">
            <div class="set-month box-cont">
                <div class="pre-month" @click="preMonth"> 《 </div>
                <div class="to-current" @click="toCurrent">当前日期</div>
                <div class="current-month">{{ currentYear }},{{ realMonth }}</div>
            ...
            <div class="days-cont ">
                <div v-for=" item in dateObj.days" class="day-item" :class="{'today': item.date == realCurrentDate ,'current-day': item.day == currentDay && item.month == currentMonth+1, 'disable-item' : item.month == currentMonth
                                        }" @click="setDay(item)">
                    {{ item.day }}
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            setDay(item) { //设置当前选择时间
                ...
                this.setDateValue = this.currentYear + '-' + this.realMonth + '-' + this.currentDay
            },
            nextMonth() { //下一月
                if (this.isSetDate) {
                    this.currentMonth--
            ...
            getDate() { //根据选择月份设置日历列表数据
                ...
                this.dateObj.month = this.currentMonth
                this.realMonth = String(+this.currentMonth + 1)
                if (this.isSetDate) {
                    this.realMonth = String(+this.currentMonth)
                    // this.isSetDate = false
                }
                this.setDateValue = this.currentYear + '-' + this.realMonth + '-' + this.currentDay
            },
            ...
            vuePicker() {
                ...
                var data1Index, data2Index, data3Index
                var monthDays = []
                picker.on('picker.change', function(index, selectedIndex) {
                    ...
                    if (index == 1) {
                        that.currentMonth = that.data2[selectedIndex].text
                        // that.realMonth = that.data2[selectedIndex].text + 1
                    }
                    if (index == 2) {
                    ...
                    that.data3 = []
                    for (var i = 1; i <= monthDays[that.currentMonth - 1]; i++) {
                        var obj = {
                            text: i,
                            value: i
                        }
                    ...
                    var setDate = that.currentYear + '-' + that.currentMonth + '-' + that.currentDay //String(+ that.currentMonth+1)
        ...
        mounted() {
            ...
            // this.nodePicker =  document.getElementById('name');
            // console.log(this.nodePicker)
            // console.log(Picker)
            this.vuePicker()
        }
    }
